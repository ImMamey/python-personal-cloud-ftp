...
class UiMainWindow(object):
    ...
    def sql_create_user(self) -> None:
        """
        Creates a user from the username and password labes, and the storage capacity from the selection box.
        :return: None
        """
        if self.linecreate_validation():
            ...
            # SQL commands and cursors
            cmdinsert_create_user = "INSERT INTO users (username, password, almacenamiento) values(?, ?, ?);"
            cmdcheck_create_user = "SELECT username FROM users WHERE username = ?;"
            cursor = db.cursor()

            # checks if there is another username in use
            cursor.execute(cmdcheck_create_user, (create_username,))
            if cursor.fetchone():
                msg = QMessageBox()
    ...
    def sql_update_user(self) -> None:
        """
        From a username, update a row in the DB.
        :return:
        """
        if self.update_user_validation():
            ...
            try:
                ...
                # CHECK for match
                cursor.execute(command_check, (old_username,))
                if cursor.fetchone():
                    try:
    ...
    def sql_delete_user(self) -> None:
        """
        Deletes a user, from a given name
        :return: None
        """
        if self.lineEdit_deleteUser.text() == "":
            ...
            # Check
            cursor.execute(command_search, (delete_username,))
            if cursor.fetchone():
                try:
