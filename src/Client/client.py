...
class MyMainGui(QWidget,Ui_Form):
    ...
    def remote_right_menu(self, pos):
        ...
        refresh = menu.addAction("Refresh")
        newfile = menu.addAction("NewFile")
        mkdir = menu.addAction("Make Dir")
        rename = menu.addAction("Rename")
            ...
            try:
                ...
                for i in range(self.Remote_Filelist.topLevelItemCount()):
                    if(str(self.Remote_Filelist.topLevelItem(i).text(0)) == dir_name[0]):
                        message = QMessageBox.information(self,'File already exists','filename already exists，please recreate')
                        return
    ...
    def connect(self):
        ...
        try:
            ...
            message = QMessageBox.information(self,'Wrong address','No response to input address，please enter again')
            self.connect()
